...
const os = require("os");
const termshell = os.platform() === "win32" ? "powershell.exe" : "bash";
const pty = require("node-pty");

var shellMap = new Map();
...
class OsController extends Controller {
   ...
   * 获取设备终端
   */
  async createTerminal(args, event) {
    const { tid, softName } = args;
    ...
    workerProcess.stdout.on('data', function (data) {
      ...
      // const trimmedStr = result.replace(/\r\n+$/, '\n');
      const trimmedStr = result
      console.log("%o", trimmedStr);
    ...
    shellMap.set(tid, workerProcess)
    return true;
  }

  async runTerminal(args, event) {
    const { tid, softName } = args;
    if (!softName) {
      return false;
    }
    ...
    // 命令行字符串 并 执行
    let workerProcess = shellMap.get(tid)
    if (!workerProcess) {
      return false;
    }
    workerProcess.stdin.write(softName)
    return true;
  }
}
