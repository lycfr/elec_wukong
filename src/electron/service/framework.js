...
class FrameworkService extends Service {
  ...
  doMemTraceTimer(type, intertime, channel, content, event, meCB) {
    // 前端ipc频道 channel
    // intertime = 10000
    console.log("type: ", type, "content: ", content, "intertime:", intertime)
    if (type == 'startMemTrace') {
      this.memTraceTimer = setInterval(function(e, c, msg) {
        meCB(e, c, msg)
      }, intertime, event, channel, content)
      return '开始了'
    } else if (type == 'startFaultTrace') {
      this.faultTraceTimer = setInterval(function(e, c, msg) {
        meCB(e, c, msg)
      }, intertime, event, channel, content)
      return '开始了'
    } else if (type == 'stopMemTrace') {
      clearInterval(this.memTraceTimer);
      return '停止了'    
    } else if (type == 'stopFaultTrace') {
      clearInterval(this.faultTraceTimer);
      return '停止了'    
    } else {
      return 'other timer not support yet'
    }
  }
