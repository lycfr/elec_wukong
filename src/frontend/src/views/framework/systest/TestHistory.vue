<template>
  <div id="app-hw-bluetooth">
    <a-collapse v-model="activeKey">
      <a-collapse-panel key="1" header="选择历史记录">
        <div class="one-block-1">
          <span>
            <a-button @click="readLogs()"> 获取历史记录 </a-button>
          </span>
        </div> 
      ...
      <a-collapse-panel key="2" header="测试结果">
        <div class="one-block-1">        
          <a-col :span="12" class="one-block-7">
            <a-statistic title="API版本" :value="apiversion" valueStyle="font-size: 16px; width: 200px; margin-right: 50px" />
          </a-col>
          <a-col :span="12" class="one-block-7">
            <a-statistic title="硬件" :value="hardware" valueStyle="font-size: 16px; width: 200px; margin-right: 50px" />
          </a-col>
          <a-col :span="12" class="one-block-7">
            <a-statistic title="软件版本" :value="version" valueStyle="font-size: 16px; width: 200px; margin-right: 50px" />
          </a-col>
          <a-col :span="12" class="one-block-7">
            <a-statistic title="开始时间" :value="testStarttime" valueStyle="font-size: 16px; width: 200px; margin-right: 50px" />
          </a-col>
        </div>
        <div class="one-block-1">
          <div class="one-block-1">        
            ...
            <a-col :span="6" class="one-block-7">
              <a-statistic title="测试时长(分钟)" :value="costTime" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
            <a-col :span="6" class="one-block-7">
              <a-statistic title="异常数(个)" :value="exceptionCnt" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
            <a-col :span="6" class="one-block-7">
              <a-statistic title="cppcrash(个)" :value="cppcrashCnt" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
            <a-col :span="6" class="one-block-7">
              <a-statistic title="jscrash(个)" :value="jscrashCnt" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
            <a-col :span="6" class="one-block-7">
              <a-statistic title="appfreeze(个)" :value="appfreezeCnt" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
            <a-col :span="6" class="one-block-7">
              <a-statistic title="memleak(个)" :value="memleakCnt" valueStyle="font-size: 16px; width: 50px; margin-right: 50px" />
            </a-col>
          </div>
        </div>
        <div class="one-block-2">
...
export default {
    ...
    return {
      ...
      rowSelection: {
        ...
        onSelect: (record, selected, selectedRows) => {
          ...
          this.currentHistoryTask = record
          store.state.currentHistoryTask = this.currentHistoryTask
          this.costTime = this.currentHistoryTask.jsonObj.costTime
          this.exceptionCnt = this.currentHistoryTask.jsonObj.exceptionCnt
          this.cppcrashCnt = this.currentHistoryTask.jsonObj.cppcrashCnt
          this.jscrashCnt = this.currentHistoryTask.jsonObj.jscrashCnt
          this.appfreezeCnt = this.currentHistoryTask.jsonObj.appfreezeCnt
          this.testDir = "logs/taskresult/"+this.currentHistoryTask.jsonObj.taskName
          this.testReport = "logs/taskresult/"+this.currentHistoryTask.jsonObj.taskName + "/wukong_report.csv"
          this.testJson = "logs/taskresult/"+this.currentHistoryTask.jsonObj.taskName + "/data.js"
          this.testException = "logs/taskresult/"+this.currentHistoryTask.jsonObj.taskName + "/filerecv.tar.gz"
          this.apiversion = this.currentHistoryTask.jsonObj.apiversion
          this.hardware = this.currentHistoryTask.jsonObj.hardware
          this.version = this.currentHistoryTask.jsonObj.version
          let date1 = new Date(this.currentHistoryTask.jsonObj.testStarttime)
          this.testStarttime = date1.toLocaleDateString().replace(/\//g, "-") + " " + date1.toTimeString().substr(0, 8)
          this.execLog = this.currentHistoryTask.jsonObj.wukongHistory
          this.top5Process = this.currentHistoryTask.jsonObj.top5Process
          this.top5Series = this.currentHistoryTask.jsonObj.top5Series
          this.initChart();
          this.resultData = this.currentHistoryTask.jsonObj.faultResult
        },
  ...
  methods: {
    ...
    getState() {
      ...
      this.currentHistoryTask = store.state.currentHistoryTask
      this.costTime = this.currentHistoryTask.jsonObj.costTime
      this.exceptionCnt = this.currentHistoryTask.jsonObj.exceptionCnt
      this.cppcrashCnt = this.currentHistoryTask.jsonObj.cppcrashCnt
      this.jscrashCnt = this.currentHistoryTask.jsonObj.jscrashCnt
    ...
    init () {
      ...
      this.$ipc.on(ipcApiRoute.readLogs, (event, result) => {
        ...
        let logList = JSON.parse(result)
        for (let i=0;i<logList.length;i++) {
          console.log("logList: ", logList[i].devid, logList[i].taskName, 
            logList[i].jsonObj.costTime, logList[i].jsonObj.exceptionCnt);
          let item = {
            taskname: logList[i].taskName,
            devid: logList[i].devid,
            costtime: logList[i].jsonObj.costTime ? logList[i].jsonObj.costTime : "N/A",
            exceptioncnt: logList[i].jsonObj.exceptionCnt ? logList[i].jsonObj.exceptionCnt : "N/A",
            jsonObj: logList[i].jsonObj
          }
          this.devidData.push(item)
        }
        store.state.logHistroy = this.devidData
        store.state.currentHistoryTask = this.devidData[0]
      })
            ...
            case "hdcListTargets":
              {
                ...
                for (let i=0;i<this.devidData.length;i++) {
                  if (this.devidData[i].name == this.currentHistoryTask) {
                    this.rowSelection.selectedRowKeys.push(i)
                    break
                  }
                }
                console.log('[ipcRenderer] [selectedDevids] result:', this.currentHistoryTask, this.rowSelection.selectedRowKeys);
              }
              break;
            case "hdcParamGet":
              {
